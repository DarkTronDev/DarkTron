...
unsigned int nTargetSpacing = 1 * 30; // DarkTron - 0.5 minute blocks
...
unsigned int nModifierInterval = 10 * 60; // DarkTron - time to elapse before new modifier is computed
...
const string strMessageMagic = "DarkTron Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
        ...
        else if(pindexBest->nHeight < 6600)
    {
        ...
        else if(pindexBest->nHeight < 6700)
    {
        nSubsidy = 100 * COIN;
    }
        ...
        else if(pindexBest->nHeight > 21000)
    {
		...
		else if(pindexBest->nHeight < 104000)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  //default 9% yr
    }
        else if(pindexBest->nHeight < 104001)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 106600)
    {
        nSubsidy = 20 * COIN;
    }
        else if(pindexBest->nHeight < 106601)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 106700)
    {
        nSubsidy = 100 * COIN;
    }
        else if(pindexBest->nHeight < 106701)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 108500)
    {
        nSubsidy = 15 * COIN;
    }
        else if(pindexBest->nHeight < 108501)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 110000)
    {
        ...
        else if(pindexBest->nHeight < 110001)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 120000)
    {
        ...
        else if(pindexBest->nHeight < 120001)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight < 121000)
    {
        nSubsidy =  50 * COIN;
    }    
        else if(pindexBest->nHeight < 121001)
    {
        nSubsidy = 500 * COIN;
    }
        else if(pindexBest->nHeight > 121002)
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  //default 9% yr
    }    
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DarkTron", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "21-05-2015 whe started the beginning";
        CTransaction txNew;
        txNew.nTime = 1432236330;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1432236330;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 14739826;
    ...
    if (true && block.GetHash() != hashGenesisBlock) {
        ...
        assert(block.hashMerkleRoot == uint256("0x5b0fabf6a8d34203d3badb52eee46eeedcc01104594f4b8e7b5c111050899724"));
